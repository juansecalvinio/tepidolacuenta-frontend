  ...
  >
    <div className="p-4 border-b border-base-300">
      <p className="text-sm text-fg-soft">
        Generá un código para que un empleado se registre con acceso solo a esa
        sucursal. Expira a los 7 días y es de un solo uso.
      </p>
    </div>
    <div className="p-4 flex flex-col gap-3">
      <div className="flex items-center gap-2 bg-base-200 rounded-lg p-3">
        ...
        <span className="btn btn-sm btn-ghost shrink-0 pointer-events-none">Copiar</span>
      </div>
      <p className="text-xs text-fg-subtle">
        Acceso a:{" "}
        <span className="font-medium text-fg-soft">Sucursal Centro</span>
      </p>
    </div>
  </div>
);
...
export const MockBranches = () => {
  const branches = [
    { name: "Sucursal Centro", address: "Av. Corrientes 1234", mesas: 12 },
    { name: "Sucursal Palermo", address: "Thames 880", mesas: 8 },
    { name: "Sucursal Costanera", address: "Av. Rafael Obligado 500", mesas: 20 },
  ];
  return (
    <div
...
export const MockTables = () => (
  ...
  >
    <div className="flex items-center justify-between mb-3">
      ...
      <span className="text-xs text-fg-subtle">Sucursal Centro</span>
    </div>
    <div className="grid grid-cols-3 sm:grid-cols-4 gap-2.5">
      {Array.from({ length: 8 }, (_, i) => i + 1).map((n) => (
        ...
        >
          ...
          <span className="text-xs font-medium">Mesa {n}</span>
        </div>
      ))}
    </div>
  </div>
...
export const MockPrintQRs = () => (
  <div className="max-w-md mx-auto lg:mx-0" aria-hidden="true">
    <div className="flex items-center justify-between mb-3">
      <span className="text-sm font-semibold text-fg-soft">Listos para imprimir</span>
    ...
    <div className="grid grid-cols-2 gap-3">
      {[
        ...
        >
          ...
          <span className="text-xs text-fg-subtle mb-3">Mesa {mesa}</span>
          <div className="rounded-lg border border-base-300 p-2.5">
            <QrGlyph className="w-16 h-16 text-fg" />
          </div>
          <span className="text-[10px] text-fg-subtle mt-2">Escaneá para pedir la cuenta</span>
        </div>
      ))}
    </div>
  </div>
